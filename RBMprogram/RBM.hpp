...
class RBM{
  ...
  public:
    ...
    void setVisiblePotentials(
	const vector<potentialType>& values){potential.at(0) = values;};
    static void setBias(const matrix<biasType>& values){bias = values;};
    static void setConnectionMatrix(
	const matrix<connectionType>& weightMatrix){
    ...
    static vector<double> batchDataMeanCalculateV(
	const matrix<potentialType>& minibatch);
...
template<typename T_traits>
double RBM<T_traits>::activationFunction(
    std::size_t layerNum, std::size_t nodeNum,
    const vector<potentialType>& nodePotentials)
{
    double sum = 0.0;
...
RBM<T_traits>::batchDataMeanCalculateVH(const matrix<potentialType>& miniBatch){
    ...
    for(std::size_t visibleNodeNum = 0;
	visibleNodeNum < totalNodeNum;++visibleNodeNum)
